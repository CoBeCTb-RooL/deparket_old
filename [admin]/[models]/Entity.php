...
class Entity2{
	...
	function getChildrenCount($essenceCode, $pid, $type)
	{
		global $_CONFIG;
	...
	{
		//vd($this);
		$tbl=Essence2::getTblName($this->essenceCode, $this->type, LANG);
			
		$sql="SELECT * FROM `".strPrepare($tbl)."` WHERE 1 AND idx > '".intval($this->idx)."' ORDER BY IDX LIMIT 1";
		//vd($sql);
		$qr=DB::query($sql);
		echo mysql_error();
		$next = mysql_fetch_array($qr, MYSQL_ASSOC);
	...
	function getPrev()
	{
		...
		$sql="SELECT * FROM `".strPrepare($tbl)."` WHERE 1 AND idx < '".intval($this->idx)."' ORDER BY idx DESC  LIMIT 1";
		//vd($sql);
		$qr=DB::query($sql);
		echo mysql_error();
		$next = mysql_fetch_array($qr, MYSQL_ASSOC);
