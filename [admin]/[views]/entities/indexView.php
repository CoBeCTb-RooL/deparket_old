...
{?>
	...
	<div class="entities">
		...
		foreach($_CONFIG['LANGS'] as $key=>$val)
			$langLinks[] = '<a href="#change_lang" id="lang-'.$key.'" class="'.($key == $_CONFIG['DEFAULT_LANG'] ? 'active' : '').'" onclick="Slonne.Entities.changeLang(\''.$key.'\'); return false;">'.$val['title'].'</a>';
		?>
		<div class="langs">
	...
	Slonne.Entities.LANG = '<?=$_CONFIG['DEFAULT_LANG']?>'
	...
	<?=Slonne::view('entities/treeElementTemplate.php');?>
